...
def get_slaves():
    ...
    return [
        ...
        worker.DockerLatentWorker('binary1248-debian',   private.slave_passwords['binary1248-debian'],   docker_host = 'unix:///var/run/docker.sock', image = 'buildbot-worker-debian',   hostconfig = {'network_mode' : 'container:sfml-ci-network'}, followStartupLogs = True, masterFQDN = 'ci.sfml-dev.org', build_wait_timeout = 0, max_builds = 1, properties = {'parallel' : 2}),
        worker.DockerLatentWorker('binary1248-android',  private.slave_passwords['binary1248-android'],  docker_host = 'unix:///var/run/docker.sock', image = 'buildbot-worker-android',  hostconfig = {'network_mode' : 'container:sfml-ci-network'}, followStartupLogs = True, masterFQDN = 'ci.sfml-dev.org', build_wait_timeout = 0, max_builds = 1, properties = {'parallel' : 1}),
        worker.DockerLatentWorker('binary1248-coverity', private.slave_passwords['binary1248-coverity'], docker_host = 'unix:///var/run/docker.sock', image = 'buildbot-worker-coverity', hostconfig = {'network_mode' : 'container:sfml-ci-network'}, followStartupLogs = True, masterFQDN = 'ci.sfml-dev.org', build_wait_timeout = 0, max_builds = 1, properties = {'parallel' : 1})
    ]
